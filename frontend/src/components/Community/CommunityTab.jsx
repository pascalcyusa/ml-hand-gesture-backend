...
export default function CommunityTab({ auth, onImportModel, onImportPiano, onImportGesture, showToast }) {
    ...
    const handleImport = useCallback(async (type, id, name) => {
        setImporting(id);
        try {
            if (type === 'model') {
    ...
    }, [onImportModel, onImportPiano, onImportGesture, pianos, gestures, showToast]);
    ...
    const renderCard = (item, type, icon, badgeVariant) => (
        <Card key={`${type}-${item.id}`} className="community-card">
            ...
            {item.description && (
                <p className="text-xs text-[var(--fg-dim)] leading-relaxed">{item.description}</p>
            )}

            {type === 'model' && (
                <div className="flex flex-wrap gap-1">
                    {item.class_names.map((cls, i) => (
                        <Badge key={i} variant={badgeVariant}>{cls}</Badge>
                    ))}
                </div>
            )}
            ...
            <div className="community-card-footer">
                ...
                <Button
                    variant="primary"
                    size="sm"
                    onClick={() => handleImport(type, item.id, item.name || item.name_or_title)}
            ...
            {loading ? (
                ...
                <div className="space-y-6">
                    ...
                    <section className="space-y-4">
                        ...
                        {expanded.models && (
                            models.length === 0 ? (
                                <p className="text-[var(--fg-muted)] italic px-2">No models found.</p>
                            ) : (
                    ...
                    <section className="space-y-4">
                        ...
                        {expanded.piano && (
                            pianos.length === 0 ? (
                                <p className="text-[var(--fg-muted)] italic px-2">No piano sequences found.</p>
                            ) : (
                                <div className="community-grid">
                    ...
                    <section className="space-y-4">
                        ...
                        {expanded.gestures && (
                            gestures.length === 0 ? (
                                <p className="text-[var(--fg-muted)] italic px-2">No motor configurations found.</p>
                            ) : (
                                <div className="community-grid">
